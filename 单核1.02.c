...
*1.1 解决了指令集部分未涉及的指令。比如对象A为内存单元 。bug：负数处理
*1.02 debug:to10;
...
void cal(int flag, char* a1, char* a2, char (*ax)[17], char* nnum, char* seg) {
	...
	else if (left >= 5 && left <= 8) mystrcpy(num1, &seg[to10(0, ax[left - 1], 16) * 8], 16);
	...
	else if (left >= 5 && left <= 8) mystrcpy( &seg[to10(0, ax[left - 1], 16) * 8], ans, 16);
}
